...
int main() {
    ...
    for(unsigned zdt = 1; zdt <= 6; zdt++) {
        ...
        solver.setWeightGeneration("grid");
        solver.setDecomposition("tchebycheff");
        solver.setNeighbours(20);
        solver.setCR(1.0);
        solver.setF(0.5);
        solver.setEtaM(20.0);
        solver.setRealb(0.9);
        solver.setLimit(2);
        solver.setPreserveDiversity(true);

        assert(fabs(solver.getTimeLimit() - 5.0) <
                std::numeric_limits<double>::epsilon());
        assert(solver.getPopulationSize() == 200);
        assert(solver.getWeightGeneration() == "grid");
        assert(solver.getDecomposition() == "tchebycheff");
        assert(solver.getNeighbours() == 20);
        assert(fabs(solver.getCR() - 1.0) <
                std::numeric_limits<double>::epsilon());
        assert(fabs(solver.getF() - 0.5) <
                std::numeric_limits<double>::epsilon());
        assert(fabs(solver.getEtaM() - 20.0) <
                std::numeric_limits<double>::epsilon());
        assert(fabs(solver.getRealb() - 0.9) <
                std::numeric_limits<double>::epsilon());
        assert(solver.getNeighbours() == 20);
